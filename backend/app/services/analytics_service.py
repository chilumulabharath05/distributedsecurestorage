...
class AnalyticsService:
    ...
    async def _get_upload_trend(
        ...
        result = await db.execute(
            select(
                func.date_trunc("day", UploadLog.created_at).label("day"),
                func.count(UploadLog.id).filter(UploadLog.action == "upload").label("uploads"),
                func.count(UploadLog.id).filter(UploadLog.action == "download").label("downloads"),
                func.coalesce(
                    func.sum(UploadLog.file_size_bytes).filter(UploadLog.action == "upload"), 0
                ).label("size"),
            )
            .where(UploadLog.user_id == user_id, UploadLog.created_at >= since)
            .group_by(func.date_trunc("day", UploadLog.created_at))
            .order_by(func.date_trunc("day", UploadLog.created_at))
        )
        return [
            {
